<template>
  <v-card
    id="music-player"
    class="mx-auto my-8 player"
    max-width="640"
  >
    <v-toolbar class="mb-2">
      <v-app-bar-nav-icon icon="mdi-radio"></v-app-bar-nav-icon>

      <v-toolbar-title class="font-gugi d-none d-sm-block ml-0">
        RADIO
      </v-toolbar-title>
      ...
      <v-btn
        v-if="!musicPlayerStore.isStreamOffline"
        :icon="musicPlayerStore.isFavorite ? 'mdi-heart' : 'mdi-heart-outline'"
        :color="musicPlayerStore.isFavorite ? 'red' : 'default'"
      ...
      >
        ...
        <v-list>
          <v-list-subheader>검색</v-list-subheader>

          <v-list-item
            :href="`https://google.com/search?q=${musicPlayerStore.trackInfo?.query}`"
            target="_blank"
          >
          ...
          <v-list-item
            :href="`https://youtube.com/results?search_query=${musicPlayerStore.trackInfo?.query}`"
            target="_blank"
          >
            <template v-slot:prepend>
      ...
      <v-chip class="font-gugi ml-1 mr-3">
        <v-icon class="mr-2" icon="mdi-headphones" size="small"></v-icon>
        {{ musicPlayerStore.listenersCount }}
      </v-chip>
    </v-toolbar>

    <div v-if="musicPlayerStore.isStreamOffline">
    ...
    <div v-else class="song-title-wrapper">
      <v-overlay
        :absolute="true"
        :value="musicPlayerStore.isLoading"
      >
        <v-progress-circular
          indeterminate
          size="48"
      ...
      <v-card-title class="text-center d-block font-noto-sans song-title">
        {{ musicPlayerStore.trackInfo?.title }}
      </v-card-title>

      <v-card-subtitle class="text-center font-noto-sans song-title">
        {{ musicPlayerStore.trackInfo?.artist }}
      </v-card-subtitle>
    </div>

    <v-card-text class="text-center mb-1 pb-0">
      <v-btn
        color="primary"
        size="x-large"
        elevation="3"
        rounded="circle"
        icon
        :loading="musicPlayerStore.isLoading"
        :disabled="musicPlayerStore.isLoading || musicPlayerStore.isStreamOffline"
        @click="musicPlayerStore.toggleRadio()"
      >
        <v-icon
          size="x-large"
          :icon="musicPlayerStore.isPaused ? 'mdi-play' : 'mdi-pause'"
        ></v-icon>
      </v-btn>
    </v-card-text>
    ...
    <v-card-actions>
      ...
      <v-btn
        plain
        href="https://live365.com/station/raste-live-a39214"
        target="_blank"
      >
    ...
    <v-dialog v-model="isRequireLogin" max-width="290">
      <v-card>
        <v-card-title>
          로그인이 필요합니다.
        </v-card-title>
    ...
    <v-dialog v-model="isNullTrackInfo" max-width="290">
      <v-card>
        <v-card-title>
          곡 정보를 불러오는데 실패했습니다.
        </v-card-title>
...
<script setup lang="ts">
  ...
  const isNullTrackInfo = ref(false)

  watch (volume, (value) => musicPlayerStore.setVolume(value))

  onMounted (() => {
    musicPlayerStore.fetchMetadata()
    musicPlayerStore.initRadio()
  })

  onUnmounted (() => {
    musicPlayerStore.disposeRadio()
  })

  function toggleFavorite () {
    if (!userStore.isLoggedIn) {
      isRequireLogin.value = true
    } else if (!musicPlayerStore.trackInfo) {
      isNullTrackInfo.value = true
    } else {
      if (musicPlayerStore.isFavorite) {
        favoriteStore.removeFavorite(musicPlayerStore.trackInfo.id)
      } else {
        favoriteStore.addFavorite(musicPlayerStore.trackInfo)
      }
    }
  }
</script>
